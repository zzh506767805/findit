...
# 后端（apps/api/**）有改动时，push 到 main 会自动触发部署，2-3 分钟生效
# 见 .github/workflows/deploy-api.yml：acr build 按 commit SHA 打 tag → containerapp update
# 凭据：GitHub secret AZURE_CREDENTIALS（服务主体 findit-api-deploy，Contributor 限 nbp-rg）
# 手动重跑：gh workflow run deploy-api.yml
...
- [ ] 新手引导
- [ ] 识别 prompt 持续调优
- [x] GitHub Actions 自动部署（后端；前端 EAS 仍手动）
