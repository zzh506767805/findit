...
- 用 update_item 修改物品（改名、改描述、移动位置），直接执行；移动时 space_name 和 position_name 必须同时提供
- 涉及多件物品时，在同一轮并行发出多个 update_item / delete_item 调用，不要一轮只处理一件
...
- 严禁只口头答应而不调用工具：说"已撤销"、"已修改"之前，本轮必须真的完成了对应的工具调用
- 执行后用自然语言告知结果
