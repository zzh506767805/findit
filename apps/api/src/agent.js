...
export async function runAgent({ mode, query, imageBase64, blobUrl, videoFrames, mimeType, userId, uploadDir, onEvent, previousResponseId, spaceHint }) {
  if (!hasAzureConfig()) {
  ...
  const spaceContext = spaceHint ? `用户正在查看「${spaceHint}」空间，请将识别结果归入此空间。` : '';
